    ...
    if(method === "GET"){
        if(path === "/jokes"){
            if(searchParams.get("num")){
                ...
                if(parseInt(num!) <= developerJokes.length){
                    return new Response(developerJokes[parseInt(num!)]);
                }
            }
            const indiceAleatorio = Math.floor(Math.random() * developerJokes.length);
            return new Response(developerJokes[indiceAleatorio]);
        }
        else if(path === "/calcular"){
            if (searchParams.get("num1") && searchParams.get("num2")&&searchParams.get("operacion")){
                const num1 = searchParams.get("num1");
                const num2 = searchParams.get("num2");
                const operacion = searchParams.get("operacion");
                if(operacion==="suma"){
                    const sum = parseInt(num1!)+parseInt(num2!);
                    return new Response(`Tu suma es: " ${(sum)}`)
                }
                else if(operacion==="resta"){
                    const resta = parseInt(num1!)-parseInt(num2!);
                    return new Response(`Tu resta es: " ${(resta)}`)
                }
                else if(operacion==="multiplicacion"){
                    const mult = parseInt(num1!)*parseInt(num2!);
                    return new Response(`Tu multiplicacion es: " ${(mult)}`)
                }
                    ...
                    return new Response(`Tu división es: " ${(div)}`)                    
                }
            }
        }
        else if(path.startsWith("/reverso/")){
            const frase = path.split("/reverso/")[1];
            const fraseReversa = frase.split('').reverse().join('');            
            if(searchParams.get("detalles")){
                const detalles = searchParams.get("detalles");
                if(detalles === "true"){
                    return new Response(`Reverso: ${(fraseReversa)} , longitud: ${(frase.length)}`)
                }
            }
            return new Response(`Reverso: ${(fraseReversa)}`);
        }
    }
